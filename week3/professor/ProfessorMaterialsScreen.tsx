...
async function generateQuizFromPDF(pdfUrl: string, courseName: string): Promise<QuizQuestion[]> {
    ...
    if (pdfUrl) {
        ...
        messageContent = [
            ...
            { type: 'text', text: `Based on this lecture PDF for the course "${courseName}", generate exactly 5 multiple-choice questions.\n\nReturn ONLY a valid JSON array (no markdown):\n[{"question":"...","options":["A","B","C","D"],"correctIndex":0}]\n\nRules: 5 questions, 4 options each, correctIndex 0-3, same language as PDF.` },
        ];
    } else {
        messageContent = `Generate 5 MCQ questions for "${courseName}". Return ONLY JSON array: [{"question":"...","options":["A","B","C","D"],"correctIndex":0}]`;
    }
            ...
            { text: 'إلغاء', style: 'cancel' },
            {
                text: 'توليد', onPress: async () => {
                    try {
                        ...
                        const questions = await generateQuizFromPDF(pdfUrl, session.courseName);
                        setQuizQuestions(questions);
                        await updateDoc(doc(db, 'sessions', session.id), { quizQuestions: questions, quizActive: false });
                    } catch (e: any) {
                            ...
                            <View>
                                ...
                                {!quizActive && (
                                    <TouchableOpacity
                                        ...
                                        {generatingQuiz
                                            ...
                                            : <><Icon name="auto-awesome" size={20} color="#fff" /><Text style={styles.generateBtnText}>{quizQuestions.length > 0 ? 'إعادة توليد' : 'توليد 5 أسئلة بالـ AI'}</Text></>
                                        }
                                    </TouchableOpacity>
                                )}
                                ...
                                {quizQuestions.length > 0 && (
                                    <View style={styles.questionsPreview}>
                                        <Text style={styles.previewTitle}>معاينة الأسئلة</Text>
