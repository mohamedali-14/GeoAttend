...
async function enrollStudent(req, res) {
    try {
        ...
        if (!studentDoc.exists) {
            return res.status(400).json({ error: 'Student not found in database. Make sure the student has registered.' });
        }
        // Accept STUDENT role or users without a role set
        const studentRole = (studentDoc.data().role || 'STUDENT').toUpperCase();
        if (studentRole !== 'STUDENT') {
            return res.status(400).json({ error: 'User is not a student (role: ' + studentRole + ')' });
        }
