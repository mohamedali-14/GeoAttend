...
const { uploadAttendanceSelfie, isSelfieRequiredForSession } = require('./selfieUpload');
...
async function joinSession(req, res) {
    try {
        ...
        if (!lat || !lng) {
            // Allow joining without GPS if session doesn't require it
            const sessionDocCheck = await db.collection('sessions').doc(sessionId).get();
            const sessionDataCheck = sessionDocCheck.exists ? sessionDocCheck.data() : null;
            if (sessionDataCheck?.geoEnabled && sessionDataCheck?.location) {
                return res.status(400).json({ error: 'Latitude and longitude are required for this session' });
            }
        }
        ...
        if (sessionLat && sessionLng && lat && lng) {
            ...
            const allowedRadius = session.radius || session.radiusMeters || defaultRadius;
            isWithinRange = distance <= allowedRadius;
        } else {
            // No geo requirement — allow
            isWithinRange = true;
            distance = 0;
        }

        if (!isWithinRange) {
            return res.status(400).json({ error: `You are ${Math.round(distance)}m away. Must be within ${session.radius || defaultRadius}m.` });
        }
