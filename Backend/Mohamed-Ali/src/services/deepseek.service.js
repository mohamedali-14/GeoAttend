...
async function submitQuizAnswers(sessionId, studentId, answers) {
    try {
        ...
        const submissionRef = await db.collection('quizSubmissions').add({
        ...
        return { submissionId: submissionRef.id, score: percentageScore, correctCount: score, totalQuestions: questions.length, results };
    } catch (error) {
