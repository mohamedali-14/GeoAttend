...
export default function StudentPanel() {
    ...
    const revokeAttendance = async () => {
      ...
      // في مكان مناسب داخل المكون (بعد useState وبعد تعريف الدوال الأخرى)
const saveScanToHistory = async (code: string, name: string, courseId: string) => {
    try {
        ...
        console.error('saveScanToHistory error:', e);
    }
};
        ...
        <SafeAreaView style={styles.container}>
            ...
            {activeSession && (
                <StudentQRScanner
                    ...
                    onScanned={(sessionId, courseId) => {
                        setScannedSessionId(sessionId);
                        setScannedCourseId(courseId);
                        setShowLectureJoin(true);
                    }}
                />
            )}
            ...
            {/* Lecture Join Screen */}
            <LectureJoinScreen
                visible={showLectureJoin}
                onClose={() => setShowLectureJoin(false)}
                sessionId={scannedSessionId}
                courseId={scannedCourseId}
                studentId={user?.uid || ''}
                studentName={studentName}
                onSuccess={(attId) => {
                    // بعد تسجيل الحضور، نحدث حالة الـ panel (مثلاً نعيد fetch البيانات)
                    fetchStudentData();
                }}
            />
...
const styles = StyleSheet.create({
    ...
    actionButton: {
        alignItems: 'center', backgroundColor: colors.background.secondary, padding: 12,
        borderRadius: 16, flex: 1, borderWidth: 1, borderColor: colors.border.primary,
    },
