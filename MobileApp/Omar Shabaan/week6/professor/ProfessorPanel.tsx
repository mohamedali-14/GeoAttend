...
export default function ProfessorDashboard() {
    ...
    const [randomCheckEnabled, setRandomCheckEnabled] = useState(false);
    ...
    const [formErrors, setFormErrors] = useState<{ course?: string; room?: string }>({});
    ...
    useEffect(() => {
        ...
        const q = query(collection(db, 'sessions'), where('professorId', '==', userData.uid));
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const sessionsData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as LectureSession[];
            setActiveSessions(sessionsData);
            if (materialsSession) {
                const updated = sessionsData.find(s => s.id === materialsSession.id);
                if (updated) setMaterialsSession(updated);
            }
        });
        return () => unsubscribe();
    }, [userData?.uid]);
    ...
    const handleCreateSession = async () => {
        ...
        try {
            ...
            Alert.alert('تم', `بدأت جلسة ${selectedCourse!.name} بنجاح!`);
            setShowNewSession(false);
            setSelectedCourse(null);
            setSessionRoom('');
            setGeoEnabled(false);
            setRadiusMeters('50');
            setRandomCheckEnabled(false);
    ...
    const handleEndSession = (id: string) => {
        Alert.alert('End Session', 'Are you sure?', [
            ...
            { text: 'End', style: 'destructive', onPress: async () => {
                try { await endSession(id); fetchData(); }
                catch (e: any) { Alert.alert('Error', e.message); }
            }},
        ]);
    };

    const formatSessionTime = (timeData: any) => {
        if (!timeData) return 'غير محدد';
        try {
            if (typeof timeData === 'string') return timeData;
            if (typeof timeData.toDate === 'function') return timeData.toDate().toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
            if (timeData instanceof Date) return timeData.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
            if (timeData.seconds) return new Date(timeData.seconds * 1000).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
            return '—';
        } catch { return '—'; }
    };
    ...
    const renderHome = () => (
        <ScrollView style={styles.tabContent} showsVerticalScrollIndicator={false}>
            <Animated.View style={[styles.heroCard, { opacity: headerOpacity }]}>
                ...
                <View style={styles.heroIcon}><Icon name="person" size={44} color={colors.primary} /></View>
            </Animated.View>
            ...
            {activeSessions.filter(s => s.isActive).length > 0 && (
                <>
                    ...
                    {activeSessions.filter(s => s.isActive).map(s => (
                        <View key={s.id} style={styles.liveCard}>
                            <View style={styles.liveBadge}>
                                ...
                                {(s as any).geoEnabled && (
                                    <View style={styles.featurePill}>
                                        <Icon name="my-location" size={11} color="#10B981" />
                                        <Text style={[styles.featurePillText, { color: '#10B981' }]}>GPS</Text>
                                    </View>
                                )}
                                {(s as any).randomCheckEnabled && (
                                    <View style={[styles.featurePill, { backgroundColor: '#8B5CF615' }]}>
                                        <Icon name="shuffle" size={11} color="#8B5CF6" />
                                        <Text style={[styles.featurePillText, { color: '#8B5CF6' }]}>Random</Text>
                                    </View>
                                )}
                                {(s as any).selfieEnabled && (
                                    <View style={[styles.featurePill, { backgroundColor: '#F59E0B15' }]}>
                                        <Icon name="face" size={11} color="#F59E0B" />
                                        <Text style={[styles.featurePillText, { color: '#F59E0B' }]}>Selfie</Text>
                                    </View>
                                )}
                            </View>
                            ...
                            <View style={styles.liveStats}>
                                ...
                                <View style={styles.liveStatItem}>
                                    ...
                                    <Text style={styles.liveStatText}>{formatSessionTime(s.startTime)}</Text>
                                </View>
                            </View>
                            <View style={styles.liveActions}>
                                ...
                                <TouchableOpacity style={[styles.liveViewBtn, { borderColor: '#8B5CF6' }]}
                                    onPress={() => { setMaterialsSession(s); setShowMaterials(true); }}>
                                    <Icon name="auto-awesome" size={16} color="#8B5CF6" />
                                    <Text style={[styles.liveViewText, { color: '#8B5CF6' }]}>PDF / Quiz</Text>
                                </TouchableOpacity>
                                <TouchableOpacity style={[styles.liveViewBtn, { borderColor: '#10B981' }]}
                                    onPress={() => { setMaterialsSession(s); setShowQuiz(true); }}>
                                    <Icon name="insert-chart" size={16} color="#10B981" />
                                    <Text style={[styles.liveViewText, { color: '#10B981' }]}>تحليل الكويز</Text>
                                </TouchableOpacity>
    ...
    return (
        <View style={styles.root}>
            <SafeAreaView style={styles.safeArea}>
                ...
                {activeTab === 'courses'    && <ProfessorCoursesScreen />}
                {activeTab === 'attendance' && <ProfessorAttendanceScreen />}
                {activeTab === 'schedule'   && <ProfessorScheduleScreen />}
            </SafeAreaView>
            ...
            <ProfessorSessionsScreen
                ...
                onToggleRandomCheck={setRandomCheckEnabled}
                selfieEnabled={selfieEnabled}
                onToggleSelfie={setSelfieEnabled}
            />

            {materialsSession && (
                <ProfessorMaterialsScreen
                    visible={showMaterials}
                    onClose={() => { setShowMaterials(false); setTimeout(() => setMaterialsSession(null), 350); }}
                    session={materialsSession}
                    professorName={userData?.fullName || ''}
...
const styles = StyleSheet.create({
    ...
    liveBadge:        { flexDirection:'row', alignItems:'center', gap:6, marginBottom:8, flexWrap:'wrap' },
    liveDot:          { width:8, height:8, borderRadius:4, backgroundColor:'#EF4444' },
    liveText:         { fontSize:11, fontWeight:'700', color:'#EF4444', letterSpacing:1 },
    featurePill:      { flexDirection:'row', alignItems:'center', gap:3, backgroundColor:'#10B98115', borderRadius:6, paddingHorizontal:6, paddingVertical:2 },
    featurePillText:  { fontSize:10, fontWeight:'700' },
    ...
    liveActions:      { flexDirection:'row', gap:8, flexWrap:'wrap' },
    liveViewBtn:      { flex:1, flexDirection:'row', alignItems:'center', justifyContent:'center', gap:6, paddingVertical:10, borderRadius:10, borderWidth:1, borderColor:colors.primary, minWidth:80 },
    liveViewText:     { fontSize:12, color:colors.primary, fontWeight:'600' },
